...
from app.agents.schemas import (
    GraphState, ABCDDiagnosticAnswer, MenziesDiagnosticAnswer,
    SPCLDiagnosticAnswer, SummaryAnswer
)
from app.agents.prompts import ABCD_PROMPT, MENZIES_PROMPT, SPCL_PROMPT, SUMMARY_PROMPT
...
class SummaryAgent:
    ...
    def summarize(self, state: GraphState):
        prompt = PromptTemplate(
            ...
            input_variables=["diagnosis_abcd", "diagnosis_menzies", "diagnosis_spcl", "image_segmentada"],
            partial_variables={"format_instructions": self.parser.get_format_instructions()},
        )
        chain = prompt | self.llm
        if "llava" not in self.model_name:
        ...
        d_abcd = state.diagnosis_abcd if isinstance(state.diagnosis_abcd, str) else state.diagnosis_abcd.model_dump_json()
        d_menzies = state.diagnosis_menzies if isinstance(state.diagnosis_menzies, str) else state.diagnosis_menzies.model_dump_json()
        d_spcl = state.diagnosis_spcl if isinstance(state.diagnosis_spcl, str) else state.diagnosis_spcl.model_dump_json()

        try:
            output = chain.invoke({
                "diagnosis_abcd": d_abcd,
                "diagnosis_menzies": d_menzies,
                "diagnosis_spcl": d_spcl,
...
def build_graph(model_name: str, retriever):
    ...
    abcd_agent = DiagnosticAgent(model_name, ABCDDiagnosticAnswer, ABCD_PROMPT, "diagnosis_abcd")
    menzies_agent = DiagnosticAgent(model_name, MenziesDiagnosticAnswer, MENZIES_PROMPT, "diagnosis_menzies")
    spcl_agent = DiagnosticAgent(model_name, SPCLDiagnosticAnswer, SPCL_PROMPT, "diagnosis_spcl")
    ...
    graph.add_node("diagnostic_abcd", abcd_agent.analyze)
    graph.add_node("diagnostic_menzies", menzies_agent.analyze)
    graph.add_node("diagnostic_spcl", spcl_agent.analyze)
